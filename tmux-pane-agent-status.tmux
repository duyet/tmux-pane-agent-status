# tmux-pane-agent-status.tmux — Tmux plugin for dynamic pane status labels.
#
...
set -g automatic-rename-format \
  "#{?pane_in_mode,[tmux],#($HOME/.tmux/plugins/tmux-pane-agent-status/scripts/pane-label.sh #{pane_id})}#{?pane_dead,[dead],}"
...
set -g status-right "#($HOME/.tmux/plugins/tmux-pane-agent-status/scripts/agent-state.sh) | %H:%M %d-%b-%y"
